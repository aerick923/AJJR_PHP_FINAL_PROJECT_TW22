...
<div class="min-h-screen flex">
    ...
    <aside class="hidden lg:flex lg:w-72 bg-slate-900 border-r border-slate-800 flex-col fixed inset-y-0">
        ...
        <nav class="flex-1 p-4 space-y-2">
            ...
            ?>
                <a href="<?= $link[1]; ?>" class="flex items-center gap-3 px-4 py-3 rounded-2xl font-bold transition
        ...
        <div class="p-4 border-t border-slate-800">
            <a href="../store.php" class="block text-center border border-slate-700 hover:border-cyan-400 px-4 py-3 rounded-xl font-bold transition mb-3">
                View Store
            </a>
            <a href="../logout.php" class="block text-center bg-red-500 hover:bg-red-400 text-white px-4 py-3 rounded-xl font-bold transition">
                Logout
            </a>
        </div>
    </aside>
    ...
    <main class="flex-1 lg:ml-72">
        <header class="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-lg border-b border-slate-800">
            <div class="px-6 py-4 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-black"><?= htmlspecialchars($page_title); ?></h2>
                    <p class="text-sm text-slate-400">Welcome, <?= htmlspecialchars($admin_name); ?></p>
                </div>

                <div class="flex items-center gap-3">
                    <a href="../store.php" class="hidden md:inline-block text-slate-300 hover:text-cyan-400 font-bold">
                        Store
                    </a>
                    <a href="../logout.php" class="bg-red-500 hover:bg-red-400 text-white px-4 py-2 rounded-xl font-bold transition">
                        Logout
                    </a>
                </div>
            </div>
            ...
            <div class="lg:hidden px-6 pb-4 flex gap-2 overflow-x-auto">
                <?php foreach ($links as $link) { 
                    $is_active = $active_page === $link[0];
                ?>
                    <a href="<?= $link[1]; ?>" class="whitespace-nowrap px-4 py-2 rounded-xl text-sm font-bold
