...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_product'])) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        $message = "Product cannot be deleted because it may already be connected to an order.";
    }
}
...
<div class="flex flex-col md:flex-row md:items-end md:justify-between gap-5 mb-8">
    ...
    <a href="addProduct.php" class="bg-cyan-400 hover:bg-cyan-300 text-slate-950 px-6 py-3 rounded-xl font-black transition">
        Add Product
    </a>
</div>
            ...
            <tbody>
                <?php while ($product = mysqli_fetch_assoc($products)) { ?>
                    <tr class="border-t border-slate-800">
                        ...
                        <td class="p-4">
                            <p class="font-black"><?= htmlspecialchars($product['product_name']); ?></p>
                            <p class="text-sm text-slate-400 max-w-md"><?= htmlspecialchars($product['description']); ?></p>
                        </td>
                        ...
                        <td class="p-4">
                            <div class="flex justify-end gap-2">
                                <a href="editProduct.php?id=<?= $product['product_id']; ?>" class="bg-blue-500 hover:bg-blue-400 text-white px-4 py-2 rounded-xl font-bold transition">
                                    Edit
                                </a>

                                <form method="POST" onsubmit="return confirm('Delete this product?');">
                                    <input type="hidden" name="product_id" value="<?= $product['product_id']; ?>">
                                    <button type="submit" name="delete_product" class="bg-red-500 hover:bg-red-400 text-white px-4 py-2 rounded-xl font-bold transition">
                                        Delete
                                    </button>
                                </form>
                            </div>
